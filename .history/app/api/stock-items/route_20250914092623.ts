...
import { PrismaClient, stock_item_status } from '@prisma/client';
  ...
  try {
    ...
    if (search) {
      where.OR = [
        ...
        { stock_items_category: { name: { contains: search, mode: 'insensitive' } } },
      ];
    }
    
    if (category) {
      where.stock_items_category = { name: category };
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !category_id || !supplier_id || !user_id) {
      return NextResponse.json(

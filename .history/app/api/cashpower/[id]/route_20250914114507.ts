...
// PATCH method for partial updates (specifically for status updates)
export async function PATCH(
  ...
  try {
    ...
    const existingTransaction = await prisma.cash_power_transaction.findUnique({
      where: { id },
    })
    ...
    if (status && !validStatuses.includes(status)) {
      return NextResponse.json(
        { 
          success: false, 
          message: 'Invalid status value. Must be one of: pending, completed, failed' 
        },
        { status: 400 }
      )
    }

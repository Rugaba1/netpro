...
import SalesManagement from "./components/manage-sales"
interface User {
  id: number
  email: string
  role: string
...
export default function NetproManagement() {
  ...
  useEffect(() => {
    ...
    const checkNotifications = async () => {
      try {
        ...
        console.log(invoiceResult, stockResult)
  ...
  const menuItems = [
    ...
    { id: "sales", label: "Sales", icon: ShoppingCart, adminOnly: false },
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "dashboard":
        currentUser.permissions.dashboard ? <Dashboard /> : null
      case "customers":
        currentUser.permissions.customers ? <CustomerManagement /> : null
      case "suppliers":
        currentUser.permissions.suppliers ? <SupplierManagement /> : null
      case "products":
        currentUser.permissions.products ? <ProductManagement /> : null
      case "packages":
        currentUser.permissions.packages ? <PackageManagement /> : null
      case "invoice":
        currentUser.permissions.invoices ? <InvoiceManagement /> : null
      case "quotation":
        currentUser.permissions.quotations ? <QuotationManagement /> : null
      case "proforma":
        currentUser.permissions.proformas ? <ProformaManagement /> : null
      case "stock":
        currentUser.permissions.stock ? <StockManagement /> : null
      case "sales":
        currentUser.permissions.sales ? <SalesManagement /> : null
      case "cashpower":
        currentUser.permissions.cashpower ? <CashpowerManagement /> : null
      case "report":
        currentUser.permissions.reports ? <Reports /> : null
      case "management":
          ...
          {filteredMenuItems.map((item) => {
            ...
            return (
              <button
                ...
                disabled={
                  (item.id === "dashboard" && !currentUser.permissions.dashboard) ||
                  (item.id === "customers" && !currentUser.permissions.customers) ||
                  (item.id === "suppliers" && !currentUser.permissions.suppliers) ||
                  (item.id === "packages" && !currentUser.permissions.packages) ||
                  (item.id === "products" && !currentUser.permissions.products) ||
                  (item.id === "invoice" && !currentUser.permissions.invoices) ||
                  (item.id === "quotation" && !currentUser.permissions.quotations) ||
                  (item.id === "proforma" && !currentUser.permissions.proformas) ||
                  (item.id === "stock" && !currentUser.permissions.stock) ||
                  (item.id === "sales" && !currentUser.permissions.sales) ||
                  (item.id === "cashpower" && !currentUser.permissions.cashpower) ||
                  (item.id === "report" && !currentUser.permissions.reports) || 
                   (item.id === "management" && currentUser.role !== "admin")}
        ...
        <header className="bg-white shadow-sm border-b h-16 flex items-center justify-between px-6">
          ...
          <div className="flex items-center space-x-4">
            <div className="relative flex items-center justify-between">
              <span className="p-4 text-center text-gray-500">Stock Balance:&nbsp;</span>
                  ...
                  <div className="p-3 border-b">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex space-x-1 justify-between items-center">
                        ...
                        <Button variant={"ghost"} className="border" onClick={()=>setShowNotifications(false)}><X className="h-3 w-3" /></Button>
                      </div>
                    </div>
                  </div>

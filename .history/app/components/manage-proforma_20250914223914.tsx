...
const ManageProforma = () => {
  ...
  const handleUpdateProforma = async () => {
    if (!editingProforma || !formData.customer_id || formData.items.length === 0) {
      toast({
        title: "Error",
        description: "Please select a customer and add at least one item",
        variant: "destructive",
    ...
    try {
      ...
      const proformaData = {
        customer_id: parseInt(formData.customer_id),
        amount_to_pay: formData.items.reduce((sum, item) => sum + item.totalIncl, 0),
        proforma_products: formData.items.map((item) => ({
          product_id: products.find((p) => p.name === item.description)?.id || 0,
          qty: item.quantity,
          unitPrice: item.unitPrice,
          price: item.totalIncl,
      ...
      const response = await axios.put(`/api/proforma-invoices/${editingProforma.id}`, proformaData);
      
      setProformas((prev) =>
        prev.map((p) => (p.id === editingProforma.id ? response.data : p))
      );
  ...
  const handleEditClick = (proforma: Proforma) => {
    ...
    setFormData({
      ...
      items: proforma.proforma_product.map((item, index) => ({
        ...
        quantity: item.qty,
        unitPrice: Number(item.unitPrice || item.product.net_price || item.product.price),
        discount: item.discount,
    ...
    calculateTotals(
      proforma.proforma_product.map((item) => ({
        quantity: item.qty,
        unitPrice: Number(item.unitPrice || item.product.net_price || item.product.price),
        discount: item.discount,
      }))
  ...
  const handleStatusChange = async (proforma: Proforma, newStatus: string) => {
    try {
      ...
      await axios.put(`/api/proforma-invoices/${proforma.id}`, {
        status: newStatus,
      });
  ...
  return (
    <div className="p-6">
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Edit Proforma Invoice #{editingProforma?.id}</DialogTitle>
          </DialogHeader>

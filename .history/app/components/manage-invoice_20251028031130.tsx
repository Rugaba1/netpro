...
export default function ManageInvoice() {
  ...
  const handleViewInvoice = (invoice: InvoiceWithRelations) => {
    ...
    const invoiceData = {
      ...
      paidAmountInLetters:
        numberToWords(Math.floor(totalIncl)) + " Rwandan Francs",
    };
  ...
  const handleCreateInvoice = async () => {
    ...
    if (createForm.isConsolidated) {
      ...
      totalAmount = createForm.invoiceItems.reduce(
        (sum: number, item: any) => sum + Number(item.amountToPay),
        0
      );
    ...
    try {
      ...
      setPendingInvoice({
        ...
        amountToPay: Number(totalAmount),
        paidAmount: Number(totalPaid),
        remainedAmount: Number(totalAmount - totalPaid),
        ...
        status:
          Number.parseFloat(paymentForm.paidAmount) === 0
            ...
            : Number(paymentForm.paidAmount) >=
              Number(pendingInvoice.amount_to_pay)
            ? "paid"
            : "partial",
      });
                ...
                {filteredInvoices.map((invoice, index) => {
                  ...
                  return (
                    <TableRow key={invoice.id}>
                      ...
                      <TableCell className="p-1 bg-red-100   rounded-md text-center font-bold">{formatCurrency(Number(invoice.amount_to_pay))}</TableCell>
                      <TableCell className="p-1 bg-green-100   rounded-md text-center font-bold">{formatCurrency(Number(invoice.amount_paid))}</TableCell>
                      <TableCell className="p-1 bg-yellow-100   rounded-md text-center font-bold">
                        {formatCurrency((
          ...
          <div className="space-y-4">
            ...
            {pendingInvoice && (
              <div className="bg-gray-50 p-3 rounded">
                <p>
                  <strong>Total Amount:</strong> {formatCurrency(Number(pendingInvoice.amountToPay))}
                </p>
                <p>
                  <strong>Paid Amount:</strong> {formatCurrency(Number(paymentForm.paidAmount))}
                </p>
                <p>

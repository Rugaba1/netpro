...
export default function ManagePackages() {
  ...
  const [editingPackage, setEditingPackage] = useState<Renamedpackage | null>(
    null
  );
  const [packageTypes, setPackageTypes] = useState<package_type[]>([]);
  const [newPackageType, setNewPackageType] = useState<package_type | null>(
    null
  );

  const [packages, setPackages] = useState<PackageWithType[]>([]);
  const [newPackage, setNewPackage] = useState<Renamedpackage | null>(null);
  const addNewPackageType = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
  ...
  const onUpdate = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      const response = await axios.put(`/api/packages/${editingPackage.id}`, {
        ...values,
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const response = await axios.post("/api/packages", {...values, type_id: values.type_id});
      if (response.status < 400) {
        getPackages()
        setNewPackage(null);
  ...
  const handleDelete = (id: number) => {
    if (confirm("Are you sure you want to delete this package?")) {
      try {
        axios.delete(`/api/packages/${id}`).then((response) => {
          if (response.status < 400) {
            getPackages();
          }
        });
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <nav className="text-sm text-gray-600 mt-1">
            <span>Home</span> / <span>Packages</span> /{" "}
            <span className="text-gray-400">Manage Package</span>
          </nav>
        </div>
      ...
      <Card>
        ...
        >
          ...
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle>Add New Package Type</DialogTitle>
              <DialogDescription>Create a new package Type</DialogDescription>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={addNewPackageType} className="space-y-4">
                <FormItem>
                ...
                <div className="flex justify-end space-x-2">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => setNewPackageType(null)}
                  >
                    Cancel
                  </Button>
                  <Button type="submit">Add Package Type</Button>
                </div>
              </form>
            </Form>
          </DialogContent>
                ...
                {filteredPackages
                  ...
                  .map((pkg, index) => (
                    <TableRow key={pkg.id} className="hover:bg-gray-50">
                      ...
                      <TableCell>
                        {pkg.created_at ? pkg.created_at.toLocaleString() : ""}
                      </TableCell>
                      <TableCell>
                        {pkg.updated_at ? pkg.updated_at.toLocaleString() : ""}
                      </TableCell>
                      <TableCell className="text-center">
                        <div className="flex justify-center space-x-1">
                          <Dialog onOpenChange={setIsEditTypeDialogOpen} open={isEditTypeDialogOpen}>
                            <DialogTrigger >
                              <Button
                                size="sm"

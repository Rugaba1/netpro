...
const ManageProforma = () => {
  ...
  const handleCreateProforma = async () => {
    ...
    try {
      ...
      const proformaData = {
        ...
        amount_paid: parseFloat(formData.amount_paid) || 0.0,
        amount_to_pay:
          total -
          (parseFloat(
            formData.items.reduce((acc, item) => acc + item.totalIncl, 0)
          ) || 0.0),
        proforma_products: formData.items.map((item, index) => ({
          product_id:
            products.find((p) => p.name === item.description)?.id || 0,
          qty: item.quantity,
  ...
  const handleMarkAsPaid = async (proforma: Proforma) => {
    try {
      ...
      setProformas((prev) =>
        prev.map((p) => (p.id === proforma.id ? { ...p, status: "paid" } : p))
      );
  ...
  const filteredProformas = proformas.filter(
    (proforma) =>
      ...
      proforma.customer.billing_name
        .toLowerCase()
          ...
          <TableBody>
            {filteredProformas.map((proforma, index) => (
              <TableRow key={proforma.id}>
                ...
                <TableCell>
                  <span
                    className={getStatusColor(proforma.status || "pending")}
                  >

...
export default function ManageInvoice() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const companiesWithCustomers = companiesRes.data.companies.map((company:any) => ({
          ...company,
  ...
  const handleCompanySelect = (companyId: string) => {
    ...
    const initialConsolidatedCustomers = companyCustomersList.map(customer => ({
      id: customer.id,
      name: customer.name,
  ...
  const handleCreateInvoice = async () => {
    if (createForm.isConsolidated) {
      // Check if we have at least one customer with items
      const hasItems = consolidatedCustomers.some(
        customer => selectedCustomers.includes(customer.id) && customer.items.length > 0
      );
      
      if (!selectedCompany || !hasItems) {
        alert("Please select a company and add at least one item to at least one customer");
        return;
      }
    } else {
      if (
    ...
    const invoiceData = {
      ...
      invoice_stock_items: createForm.isConsolidated
        ...
        : createForm.invoiceItems.map((item: any, index: number) => ({
            item_id: items.find(i => i.name === item.name)?.id || 0,
            qty: 1,
            user_id: 1 // Replace with actual user ID from context
          }))
    ...
    try {
      ...
      setPendingInvoice({
        ...
        productType: createForm.isConsolidated
          ? consolidatedCustomers.flatMap(c => c.items.map(i => i.name)).join(", ")
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                      ...
                      <SelectContent>
                        {items.map((item) => (
                          <SelectItem key={item.id} value={item.id.toString()}>
                            {item.name} - {item.product.net_price  || item.product.price } RWF
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                        ...
                        {consolidatedCustomers.map(customer => (
                          <div key={customer.id} className="border rounded">
                            <div className="flex items-center p-3 bg-gray-50">
                              ...
                              <Button
                                size="sm"
                                onClick={() => handleAddItemToCustomer(customer.id)}
                                        ...
                                        <TableRow key={item.id}>
                                          ...
                                          <TableCell>
                                            <Button
                                              size="sm"
                                              variant="destructive"
                                              onClick={() => handleRemoveItemFromCustomer(customer.id, item.id)}
                                            >
                                              <Trash className="h-3 w-3" />
                                            </Button>
                                          </TableCell>
                      ...
                      <div className="mt-4 p-3 bg-blue-50 rounded">
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            <p>
                              <strong>Total Amount:</strong>{" "}
                              {calculateConsolidatedTotals().totalAmount.toLocaleString()}{" "}
                              RWF
                            </p>
                            <p>
                              <strong>Total Paid:</strong>{" "}
                              {calculateConsolidatedTotals().totalPaid.toLocaleString()}{" "}
                              RWF
                            </p>
                          </div>
                          <div>
                            <p>
                              ...
                              {selectedCustomers.length} of {consolidatedCustomers.length}
                            </p>
                            <p>
                              <strong>Total Items:</strong>{" "}
                              {consolidatedCustomers.reduce((sum, customer) => 
                                selectedCustomers.includes(customer.id) ? sum + customer.items.length : sum, 0
                              )}
                            </p>
                          </div>
                        </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                createForm.isConsolidated 
                  ? !selectedCompany || consolidatedCustomers.filter(c => selectedCustomers.includes(c.id) && c.items.length > 0).length === 0
                  : createForm.invoiceItems.length === 0
              }
            >
              Create Invoice

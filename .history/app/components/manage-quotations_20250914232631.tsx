...
const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  const filteredQuotations = quotations.filter(
    (quotation) =>
      ...
      quotation.customer.billing_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const productTotal = (selectedProduct.net_price || selectedProduct.price) * newProduct.qty;
  ...
  const handleRemoveProduct = (productId: number, isEdit = false) => {
    if (isEdit) {
      setEditForm((prev) => ({
        ...prev,
        quotation_products: prev.quotation_products.filter((item) => item.id !== productId),
      }));
    } else {
      setCreateForm((prev) => ({
        ...prev,
...
const handleEditQuotation = async () => {
  if (!editingQuotation || !editForm.customer_id || editForm.quotation_products.length === 0) {
    alert("Please select a customer and add at least one product");
    return;
  }
  ...
  try {
    ...
    const response = await axios.put(`/api/quotations/${editingQuotation.id}`, quotationData);
  ...
  const handleDeleteQuotation = async (quotation: Quotation) => {
    if (!confirm(`Are you sure you want to delete quotation #${quotation.id}?`)) {
      return;
    }
 ...
 const handleEditClick = (quotation: Quotation) => {
  ...
  const handlePrintQuotation = (quotation: Quotation) => {
    ...
    const printContent = `
      ...
      <body>
        ...
        <div class="total">
          <h3>Grand Total: RWF ${quotation.quotation_product
                ...
                {filteredQuotations.map((quotation) => {
                  ...
                  return (
                    <TableRow key={quotation.id}>
                      ...
                      <TableCell>RWF {totalAmount.toLocaleString()}</TableCell>
                      <TableCell>
            ...
            <div className="border rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                ...
                <div className="flex items-end">
                  <Button onClick={() => handleAddProduct(false)} className="w-full">
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-blue-600">
                        RWF {calculateTotal(createForm.quotation_products).toLocaleString()}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="border rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                ...
                <div className="flex items-end">
                  <Button onClick={() => handleAddProduct(true)} className="w-full">
                    <Plus className="h-4 w-4 mr-2" />
                    Add Product
                  </Button>
                </div>
                ...
                <div className="mt-4 p-3 bg-blue-50 rounded">
                  <div className="flex justify-between items-center">
                    <div>
                      <p className="font-semibold">Grand Total</p>
                      <p className="text-2xl font-bold text-blue-600">
                        RWF {calculateTotal(editForm.quotation_products).toLocaleString()}
                      </p>
                    </div>
                  </div>
                </div>
  ...
  className="bg-blue-600 hover:bg-blue-700"
  disabled={editForm.quotation_products.length === 0 || isSubmitting}
>
              ...
              <div className="bg-gray-50 p-4 rounded">
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    RWF{" "}
                    {selectedQuotation.quotation_product
                      .reduce((sum, item) => sum + item.price, 0)
                      .toLocaleString()}
                  </p>
                </div>

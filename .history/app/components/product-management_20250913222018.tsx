...
export default function ProductManagement() {
  ...
  const fetchProductTypes = async () => {
    try {
      ...
      if (isAxiosError(error)) {
        setError(error.response?.data?.message || "Failed to fetch product types");
      } else {
        setError("An unexpected error occurred");
      }
    }
  ...
  const filteredProducts = products.filter(
    (product) =>
      product.package?.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      product.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      product.type.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Form submission handlers
  const handleProductSubmit = async (values: z.infer<typeof productFormSchema>) => {
    try {
  ...
  const handleProductTypeSubmit = async (values: z.infer<typeof productTypeFormSchema>) => {
    try {
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      try {
        await Promise.all([fetchPackages(), fetchProducts(), fetchProductTypes()]);
      } catch (error) {
              ...
              <Dialog open={isAddProductTypeDialogOpen} onOpenChange={setIsAddProductTypeDialogOpen}>
                ...
                <DialogContent className="sm:max-w-[425px]">
                  ...
                  <Form {...productTypeForm}>
                    <form
                      onSubmit={productTypeForm.handleSubmit(handleProductTypeSubmit)}
                      className="space-y-4"
                    >
                      <FormField
                  ...
                  filteredProducts
                    ...
                    .map((product, index) => (
                      <TableRow key={product.id} className="hover:bg-gray-50">
                        ...
                        <TableCell>{product.package?.description || "N/A"}</TableCell>
                        <TableCell>{product.type.name}</TableCell>
                        <TableCell>${product.price.toNumber().toFixed(2)}</TableCell>
                        <TableCell>${product.net_price.toNumber().toFixed(2)}</TableCell>
          ...
          {!isLoading && filteredProducts.length > 0 && (
            <div className="flex justify-between items-center mt-4 flex-wrap gap-4">
              <div className="text-sm text-gray-600">
                Showing 1 to{" "}
                {Math.min(Number.parseInt(showEntries), filteredProducts.length)} of{" "}

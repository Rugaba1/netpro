...
import { Search, Plus, Zap, DollarSign, Users, TrendingUp, Loader2, X, CheckCircle, Download, Filter, Eye, Printer } from "lucide-react"
import { toast } from "sonner"
import { cash_power_transaction as CashpowerTransactionType, customer as Customer  } from "@/lib/generated/prisma"
...
export default function CashpowerManagement() {
  ...
  const exportTransactions = () => {
    ...
    const headers = "Customer,Meter Number,Amount,Units,Token,Commission,Status,Date\n"
  ...
  const totalCommission = transactions.reduce((sum, t) => sum + parseFloat((t?.commission ?? 0).toString()), 0)
        ...
        <CardContent className="p-6">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" className="flex items-center gap-2" onClick={exportTransactions}>
        ...
        <CardContent>
          {filteredTransactions.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              No transactions found. {searchTerm && "Try adjusting your search."}
            </div>
          ) : (
            <>
                  ...
                  <TableBody>
                    {filteredTransactions.map((transaction) => (
                      <TableRow key={transaction.id}>
                        ...
                        <TableCell className="font-mono">{transaction.meter_no}</TableCell>
                        <TableCell className="font-medium">RWF {parseFloat(transaction.amount.toString()).toLocaleString()}</TableCell>
                        <TableCell>{transaction.units?.toFixed(1) || 'N/A'} kWh</TableCell>
                        <TableCell>
                          <div className="font-mono text-sm bg-gray-100 p-2 rounded">{transaction.token || "N/A"}</div>
                        </TableCell>
                        <TableCell className="text-green-600 font-medium">RWF {parseFloat((transaction.commission ?? 0).toString()).toLocaleString()}</TableCell>
                        <TableCell>
                          <Badge className={`${getStatusColor(transaction?.status ?? "unknown")} text-white`}>

...
export default function ManageInvoice() {
 ...
 const handleViewInvoice = (invoice: InvoiceWithRelations) => {
  const items = invoice.invoice_stock_item.map((item, index) => ({
    ...
    vat: Number(item.item.product.price) - (Number(item.item.product.price) / 1.18),
...
const handleCreateInvoice = async () => {
  ...
  const invoiceData = {
    ...
    customer_id: createForm.isConsolidated ? null : parseInt(createForm.customerName),
                ...
                {filteredInvoices.map((invoice, index) => (
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell className="min-w-[150px]">
                      {invoice.master_invoice?.invoice_type === "consolidated"
                    ...
                    <TableCell>
                      {(invoice.amount_to_pay - invoice.amount_paid).toString()}
                    </TableCell>
                    <TableCell>

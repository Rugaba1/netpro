...
export default function ManageInvoice() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const [invoicesRes, customersRes, itemsRes, companiesRes] =
          await Promise.all([
  ...
  const handleViewInvoice = (invoice: InvoiceWithRelations) => {
    const items = invoice.invoice_stock_item.map((item, index) => ({
      ...
      priceExclVat:
        Number(item.item.product.net_price || item.item.product.price) / 1.18,
      vat:
        Number(item.item.product.net_price || item.item.product.price) -
        Number(item.item.product.net_price || item.item.product.price) / 1.18,
      totalIncl:
        Number(item.item.product.net_price || item.item.product.price) *
        item.qty,
              ...
              <TableBody>
                {filteredInvoices.map((invoice, index) => (
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell>
                      {(
                        invoice.amount_to_pay.toNumber() -
                        invoice.amount_paid.toNumber()
                      ).toString()}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      {invoice.start_date.toString().split("T")[0]}
                    </TableCell>
                    <TableCell>
                      {invoice.end_date.toString().split("T")[0]}
                    </TableCell>
                    <TableCell>
                      ...
                      <SelectContent>
                        {items.map((item) => (
                          <SelectItem key={item.id} value={item.id.toString()}>
                            {item.name} -{" "}
                            {item.product.net_price || item.product.price} RWF
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                        ...
                        >
                          ...
                          <SelectContent>
                            {items.map((item) => (
                              ...
                              >
                                {item.name} -{" "}
                                {item.product.net_price || item.product.price}{" "}
                                RWF
                              </SelectItem>
                            ))}
                          </SelectContent>
